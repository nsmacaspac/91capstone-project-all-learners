...
The Baseline Algorithm was constructed on the basic assumption that the average rating $\mu$ of 3.5124 in the train set predicts ratings with the least error from the actual ratings in the testing set. This gave an RMSE of 1.0593, which served as the basis of comparison for the RMSE of the succeeding algorithms (tab. 1).
...
The Baseline Algorithm predicted rating was improved by adding the deviation from the average caused by the observed movie bias. The bias of each movie $b_i$ was estimated from the average of the movie ratings less the effect of the $\mu$ (Irizarry 2022). This lowered the RMSE of the prediction to 0.94292 (tab. 1).
...
The Algorithm 1 predicted ratings were amended with the deviation from the average caused by the observed genre bias. The bias of each genre combination $b_g$ was estimated from the average of the genre-combination ratings less the effects of the $\mu$ and $b_i$. However, this did not significantly affect the RMSE of the prediction, which remained at 0.94292 (tab. 1).
...
Hence, genres were arranged from highest to lowest rating. Then, the top genre was extracted from each genre combination with the str_detect function. The $b_g$ was recalculated from the average of the top-genre ratings less the effects of the $\mu$ and $b_i$. Still, this did not significantly affect the RMSE of the prediction, which remained at 0.94292 (tab. 1).
...
The Algorithm 1 predicted ratings were updated with the deviation from the average caused by the observed bias from the years of release in place of that from the genres. The bias of each year of release $b_r$ was estimated from the average of the year-of-release ratings less the effects of the $\mu$ and $b_i$. However, this did not significantly affect the RMSE of the prediction as well, which remained at 0.94292 (tab. 1).
...
The Algorithm 1 predicted ratings were enhanced by adding the deviation from the average caused by the observed user bias in place of that from the year of release. The bias of each user $b_u$ was estimated from the average of the user ratings less the effects of the $\mu$ and $b_i$ (Irizarry 2022). This further lowered the RMSE of the prediction to 0.86458, which narrowly breached the required RMSE of 0.86490 (tab. 1).
...
The Algorithm 4 predicted ratings were enhanced by adding the deviation from the average caused by the last observed bias from the relative ages of movies. The bias of each relative age $b_a$ was estimated from the average of the relative-age ratings less the effects of the $\mu$, $b_i$ and $b_u$. This slightly lowered the RMSE of the prediction to 0.86414, which is below the required RMSE of 0.86490 (tab. 1).
...
The movie bias and those of other predictors in Algorithm 5 were adjusted for the number of ratings using regularization, which penalizes big estimates from small sample sizes with $\lambda$ and avoids overfitting of the algorithm (Koren 2008, 427; Koren 2009, 2). Cross-validation resulted in a $\lambda$ of 5.3. This was used for the regularization of the biases, which refined the RMSE to 0.86353 (tab. 1).
...
The final Algorithm 6 formed the recommendation system for this project. It predicted ratings ($r_p$) starting with an average rating 3.5124 then additionally adjusting for the effects of the movie, user and age biases that were regularized with a $\lambda$ of 5.3: $r_p = 3.5124 + rb_i + rb_u + rb_a$. The movie and user biases accounted for majority of the effects, similar to what was observed in previous data (Koren 2008, 427). Nevertheless, the age bias helped achieve an RMSE of 0.86353.

A previous project by McGinnis (2019) similarly reviewed the use of movie age and relative age in recommendation systems. However, the pattern used to extract the years of releases from the titles yielded erroneous years. Both movie age and relative age were not used in the final algorithm.

Finally, the recommendation system was evaluated on the reserved final holdout test set to simulated the movie rating prediction of the recommendation system on future data. This resulted in a decent RMSE of 0.86469, which is below the required RMSE of 0.86490 (tab. 1). 
...
In this project, a content-based recommendation system that predicted future movie ratings of users with an RMSE rate of 0.86469 was developed from the MovieLens 10M Dataset. As with the Netflix challenge, the RMSE can be pushed down further using advanced methods, such as collaborative filtering of user-movie interactions. However, this goes beyond the scope of this project. It would be interesting to know if more predictor data has been added to the MovieLens dataset in 2020 to 2021, when most people were confined to their homes with their movies.
